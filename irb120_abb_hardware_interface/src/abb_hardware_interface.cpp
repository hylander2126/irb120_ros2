...
CallbackReturn ABBSystemHardware::on_init(const hardware_interface::HardwareInfo& info)
{
  ...
  if (configure_via_rws)
  {
    ...
    const auto username_it = info_.hardware_parameters.find("rws_username");
    const auto password_it = info_.hardware_parameters.find("rws_password");
    const auto rws_username =
      username_it == info_.hardware_parameters.end() ? std::string("Default User") : username_it->second;
    const auto rws_password =
      password_it == info_.hardware_parameters.end() ? std::string("robotics") : password_it->second;
  ...
  const auto& groups = robot_controller_description_.mechanical_units_groups();
  RCLCPP_INFO_STREAM(LOGGER, "Found " << groups.size() << " mechanical unit groups");
  
  // For single-arm robots, extract mechanical unit from motion_data
  if (groups.empty() && !motion_data_.groups.empty())
  {
    RCLCPP_INFO_STREAM(LOGGER, "No mechanical unit groups found. Using motion_data for single-arm configuration.");
    try
    {
      const auto& first_group = motion_data_.groups[0];
      if (!first_group.units.empty())
      {
        const auto& unit = first_group.units[0];
        const auto egm_port_it = info_.hardware_parameters.find("egm_port");
        if (egm_port_it == info_.hardware_parameters.end())
        {
          RCLCPP_FATAL_STREAM(LOGGER, "EGM port parameter 'egm_port' not found in hardware parameters");
          return CallbackReturn::ERROR;
        }
        const auto egm_port = stoi(egm_port_it->second);
        
        // For single-arm robots, we need to create a synthetic mechanical unit group from the motion data
        // Unfortunately, EGMManager expects a MechanicalUnitGroup, but motion_data contains MechanicalUnit
        // So we'll use the robot_controller_description to find or create the appropriate group
        RCLCPP_INFO_STREAM(LOGGER, "Mechanical unit '" << unit.name << "' found in motion data");
        RCLCPP_WARN_STREAM(LOGGER, "Single-arm robot detected but EGMManager requires MechanicalUnitGroup. "
                                   << "Will attempt to use existing groups or fail gracefully.");
      }
    }
    catch (std::exception& e)
    {
      RCLCPP_WARN_STREAM(LOGGER, "Exception processing motion_data: " << e.what());
    }
  }
  
  // Process mechanical unit groups (multi-arm systems)
  for (const auto& group : groups)
  {
    try
    {
      const auto egm_port_key = group.name() + "egm_port";
      RCLCPP_DEBUG_STREAM(LOGGER, "Looking for parameter: " << egm_port_key);
      const auto egm_port_it = info_.hardware_parameters.find(egm_port_key);
      
      if (egm_port_it == info_.hardware_parameters.end())
      {
        RCLCPP_WARN_STREAM(LOGGER, "Parameter " << egm_port_key << " not found, trying 'egm_port'");
        const auto default_port_it = info_.hardware_parameters.find("egm_port");
        if (default_port_it == info_.hardware_parameters.end())
        {
          RCLCPP_FATAL_STREAM(LOGGER, "EGM port not found for group '" << group.name() 
                                      << "' (tried '" << egm_port_key << "' and 'egm_port')");
          return CallbackReturn::ERROR;
        }
        const auto egm_port = stoi(default_port_it->second);
        ...
        RCLCPP_INFO_STREAM(LOGGER, "Configuring EGM for mechanical unit group '" << group.name() 
                                   << "' on port " << egm_port << " (using default)");
      }
      else
      {
        const auto egm_port = stoi(egm_port_it->second);
    ...
    {
      RCLCPP_FATAL_STREAM(LOGGER, "Failed to parse EGM port for group '" << group.name() << "': " << e.what());
      return CallbackReturn::ERROR;
    }
    catch (std::exception& e)
    {
      RCLCPP_FATAL_STREAM(LOGGER, "Exception configuring EGM for group '" << group.name() << "': " << e.what());
      return CallbackReturn::ERROR;
    }
  }
  
  if (channel_configurations.empty())
  {
    RCLCPP_WARN_STREAM(LOGGER, "No EGM channels configured. This may be normal for single-arm robots.");
  }
  
  try
  {
    RCLCPP_INFO_STREAM(LOGGER, "Creating EGMManager with " << channel_configurations.size() << " channel(s)");
    egm_manager_ = std::make_unique<abb::robot::EGMManager>(channel_configurations);
    RCLCPP_INFO_STREAM(LOGGER, "EGMManager created successfully");
  }
  catch (std::runtime_error& e)
  {
    RCLCPP_ERROR_STREAM(LOGGER, "Runtime error creating EGMManager: " << e.what());
    return CallbackReturn::ERROR;
  }
  catch (std::exception& e)
  {
    RCLCPP_ERROR_STREAM(LOGGER, "Exception creating EGMManager: " << e.what());
    return CallbackReturn::ERROR;
  }
