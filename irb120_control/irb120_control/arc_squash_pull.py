...
class ArcSquashPull(Node):
    ...
    def _arc_fx_flipped(self, angle: float) -> bool:
        """Return True once fx has crossed opposite the dominant ARC sign."""
        if self._arc_start_angle is None:
            return False
        swept = abs(self._arc_start_angle - angle)
        if swept < math.radians(ARC_FX_SIGN_MIN_SWEEP_DEG):
            return False
        if abs(self._force_x) < ARC_FX_SIGN_DEADBAND_N:
            self._arc_fx_flip_count = 0
            return False

        sign = 1 if self._force_x > 0.0 else -1
        if sign > 0:
        ...
        if self._arc_fx_majority_sign is None and total >= ARC_FX_SIGN_MIN_SAMPLES:
            ...
            self.get_logger().info(
                f"ARC fx majority sign locked: {'+' if self._arc_fx_majority_sign > 0 else '-'} "
    ...
    def _tick(self) -> None:
        ...
        if self._state == "ARC":
            ...
            if self._arc_fx_flipped(angle):
                ...
                self.get_logger().info(
                    f"fx sign flip reached: fx={self._force_x:.2f} N "
            ...
            if angle <= self._arc_end_angle:
                ...
                self.get_logger().warn(
                    ...
                    f"(fx={self._force_x:.2f} N); entering LULL as safety fallback"
                )
                self._transition("LULL")
            return
