...
class ArcStatic(Node):
    ...
    def _publish_arc_step(
        ...
        vy = self._vy_force()
        if not self._servo_cmd.publish_twist(vx, vy, vz, 0.0, self._state, f_radial):
            self._done = True
...
def main(args=None) -> int:
    ...
    try:
        ...
        try:
            save_ft_pose_log(node._ft_transformed_log, node._pose_log, node._log_subdir, "arc_static", node._obj_pose_log)
